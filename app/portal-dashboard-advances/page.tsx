...
import { Menu, X, LogOut, ChevronDown, Zap, ChartNoAxesColumn } from 'lucide-react'
import { FundoLogo } from '@/components/fundo-logo'
import { PortalDashboardAdvancesSkeleton } from '@/components/portal-dashboard-advances-skeleton'

export default function PortalDashboardAdvancesPage() {
  ...
  return (
    <div className="bg-[#e8e8e8] flex flex-col min-h-screen">
      ...
      {isSidebarOpen && (
        ...
        {/* Main Content */}
        <main className="flex-1 px-4 py-4 lg:py-8">
          {showSkeleton ? (
            <PortalDashboardAdvancesSkeleton />
          ) : (
            <>
          {/* Main Content Container */}
          <div className="bg-transparent lg:bg-white rounded-2xl border p-6 lg:p-8 w-full md:max-w-[600px] xl:max-w-[1100px] border-gray-200 shadow-none lg:shadow-md px-0 py-0 md:mx-auto lg:ml-0 lg:mt-20">
            {/* Header with Title and Profile */}
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-8">
              {/* Welcome Section */}
              <div className="mb-6 lg:mb-0">
                <h2 className="mb-2 font-serif lg:text-3xl font-normal text-center lg:text-left text-4xl text-blue-950">Hi Leslie Knope</h2>
                <p className="font-sans text-center text-slate-900 text-sm">Welcome back! – everything&apos;s ready for you to pick up where you left off.</p>
              </div>

              {/* Profile Dropdown - Desktop only */}
              <div className="hidden lg:block lg:relative" ref={dropdownRef}>
                <button
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            ...
            <div className="space-y-6">
              ...
              <Link href="/portal-advances-detail" className="block p-6 lg:p-8 rounded-2xl border border-gray-200 bg-white lg:max-w-[357px] lg:px-4 lg:py-4 shadow-md hover:shadow-lg transition-shadow cursor-pointer">
                {/* Header with Icon */}
                <div className="flex items-center gap-3 mb-5">
                  <div className="rounded-full bg-[#fcc419] flex items-center justify-center flex-shrink-0 w-9 h-9">
                    <ChartNoAxesColumn width="24" height="24" className="w-6 h-6 text-gray-900" />
                  </div>
                  <h3 className="text-gray-500 font-sans text-base">Your current balance</h3>
                </div>
                  ...
                  <div className="flex flex-col items-center gap-4">
                    <div className="relative w-40 h-40">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-4xl font-bold text-[#009951]">50%</span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-gray-600 text-sm font-sans">Payment Progress</p>
                      <span className="inline-block bg-[#8dcff9] text-[#0d7bc5] font-semibold text-sm py-1.5 rounded-sm px-1">6 of 12</span>
                    </div>
                  </div>

                  {/* Right Content */}
                  <div className="flex-1 grid grid-cols-2 lg:flex lg:flex-col gap-6 lg:space-y-6 lg:gap-0">
                    {/* Total Due */}
                    <div>
                      <p className="text-gray-600 text-sm font-sans mb-1">Total Due</p>
                      <p className="text-4xl font-bold text-gray-900 font-serif">$1,450</p>
                    </div>

                    {/* Still to Pay */}
                    <div>
                      <p className="text-gray-600 text-sm font-sans mb-1">Still to Pay</p>
                      <p className="text-3xl font-bold text-[#009951] font-serif">$1,000</p>
                    </div>
                  </div>
                </div>
